...
update_meter(Now, open, #time{send=Send} = Time, Q, V,
    ...
    try VMod:open_time(V) of
        ...
        Class:Reason:Stack ->
            valve_exception(Class, Reason, open, Time, Q, V, Config, Stack)
    end;
update_meter(Now, closed, #time{send=Send, empty=Empty} = Time, Q, V,
    ...
    try QMod:send_time(Q) of
        ...
        Class:Reason:Stack ->
            queue_exception(Class, Reason, closed, Time, Q, V, Config, Stack)
    end.
...
closed({continue, From, Ref}, #time{send=Send, now=Now} = Time, Q, V, _,
    ...
    try VMod:handle_continue(Ref, Send, V) of
        ...
        Class:Reason:Stack ->
            valve_exception(Class, Reason, closed, Time, Q, V, Config, Stack)
    end;
...
common({update, From, Value}, State, #time{send=Send} = Time, Q, V, _,
    ...
    try VMod:handle_update(Value, Send, V) of
        ...
        Class:Reason:Stack ->
            valve_exception(Class, Reason, State, Time, Q, V, Config, Stack)
    end;
...
common({len, From, _}, State, Time, Q, V, _,
    ...
    try QMod:len(Q) of
        ...
        Class:Reason:Stack ->
            queue_exception(Class, Reason, State, Time, Q, V, Config, Stack)
    end;
...
opening_out(#time{now=Now, send=Send} = Time, Q, V, VNext,
    ...
    try QMod:handle_out(Now, Q) of
        ...
        Class:Reason:Stack ->
            queue_exception(Class, Reason, open, Time, Q, V, Config, Stack)
    end.
